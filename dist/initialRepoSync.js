...
if (!githubToken || !openaiApiKey || !repoFullName || !vectorStoreId) {
    console.error("Missing required environment variables: GITHUB_TOKEN, OPENAI_API_KEY, REPO_FULL_NAME, VECTOR_STORE_ID");
    process.exit(1);
}
const octokit = new Octokit({ auth: githubToken });
const openai = new OpenAI({ apiKey: openaiApiKey });
async function listAllFiles(owner, repo, ref = "master", dir = "") {
    const files = [];
...
async function fetchFileContent(owner, repo, filePath, ref = "master") {
    try {
        const response = await octokit.repos.getContent({
            owner,
            repo,
...
async function main() {
    ...
    for (const file of supportedFiles) {
        ...
        if (content) {
            batch.push({ filename: file, status: "added", content });
        }
    }
